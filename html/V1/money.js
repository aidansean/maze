...
function gold_piece_object(amount){
  ...
  this.draw = function(context,i,j){
    ...
    context.fillText(this.amount,cx,cy+0.35*fontSize) ;
  }
}
...
function count_gold_on_map(){
  ...
  for(var B=0 ; B<master_map.length ; B++){
    for(var A=0 ; A<master_map[B].length ; A++){
      for(var j=0 ; j<master_map[B][A].cells.length ; j++){
        for(var i=0 ; i<master_map[B][A].cells[j].length ; i++){
          switch(master_map[B][A].cells[j][i]){
            case 21: gp +=1 ; break ;
            case 22: gp +=2 ; break ;
            case 23: gp +=5 ; break ;
          }
