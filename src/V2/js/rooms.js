...
function room_object(cells,cell_contents,name,wallEdgeColor,wallFillColor,floor_type){
  ...
  if(undefined==cell_contents){
    this.cell_contents = new Array() ;
    for(var i=0 ; i<nCol ; i++){
      this.cell_contents.push(new Array()) ;
      for(var j=0 ; j<nRow ; j++){
        this.cell_contents[i][j] = 0 ;
      }
    }
  }
  else{
  ...
  this.clone = function(){
    ...
    var cell_contents_clone = new Array() ;
    for(var j=0 ; j<nRow ; j++){
    ...
    for(var j=0 ; j<nRow ; j++){
      cell_contents_clone.push(new Array()) ;
      for(var i=0 ; i<nCol ; i++){
        cell_contents_clone[j].push(this.cell_contents[j][i]*1) ;
      }
    }
    room_clone.cell_contents = cell_contents_clone ;
  ...
  this.write_output = function(){
    ...
    for(var i=0 ; i<this.cells.length ; i++){
      ...
      for(var j=0 ; j<this.cells[i].length ; j++){
        ...
        if(j==this.cells[i].length-1){ str_row.push(value) ; }
        else{ str_row.push(value+',') ; }
      }
      str_row.push(']') ;
    ...
    str.push('      ],') ;
    str.push("      [") ;
    for(var i=0 ; i<this.cell_contents.length ; i++){
      var str_row = [] ;
      str_row.push('       [') ;
      for(var j=0 ; j<this.cell_contents[i].length ; j++){
        var value = this.cell_contents[i][j] ;
        if(value<10) value = ' '+value ;
        if(j==this.cell_contents[i].length-1){ str_row.push(value) ; }
        else{ str_row.push(value+',') ; }
      }
      str_row.push(']') ;
      if(i!=this.cell_contents.length-1) str_row.push(',') ;
      str_row = str_row.join('') ;
      str.push(str_row) ;
    }
    str.push('      ],') ;
  ...
  this.load0  = function(){ // Called for all rooms.  Special rooms calls this
    control_listeners = [] ;
  ...
  this.load = function(){ return this.load0() ; }
  this.unload = function(){ return ; }
}

function cell_object(type, contents){
  this.type = type ;
...
function get_cell_contents(A,B,i,j){
  ...
  var room_tmp = get_room(A,B) ;
  return room_tmp.cell_contents[j][i] ;
}
...
function set_cell_contents(A,B,i,j,value){
  if(i< 0   ){ i = nCol+i ; A-- ; }
  if(i>=nCol){ i = i-nCol ; A++ ; }
  if(j< 0   ){ j = nRow+j ; B-- ; }
  if(j>=nRow){ j = j-nRow ; B++ ; }
  var room_tmp = get_room(A,B) ;
  room_tmp.cell_contents[j][i] = value ;
}
...
function reset_room(){
  ...
  cell_contents[player.j][player.i] = 99 ;
  // Reset pressure pads
  for(var i=0 ; i<pad_and_door_objects.length ; i++){
    var p = pad_and_door_objects[i] ;
...
function get_room_by_name(name){
  for(var i=0 ; i<rooms.length ; i++){
    for(var j=0 ; j<rooms[i].length ; j++){
      if(rooms[i][j].name==name) return rooms[i][j] ;
    }
  }
  return null ;
}
